...
export default function SignUp() {
  ...
  const initSignUp = async () => {
    ...
    setLoading(true);
    try {
      await apiPost("/sign-up/init", { name, email, password });
      setOk("OTP sent to your email. Check your inbox.");
      setStep("otp");
    } catch (e) {
      setErr(e.message);
    } finally {
  ...
  const completeSignUp = async () => {
    ...
    setLoading(true);
    try {
      await apiPost("/sign-up/complete", { email, otp });
  ...
  return (
    <div style={s.page}>
      <div style={s.card}>
        ...
        <div style={s.sub}>{step === "init" ? "Enter your details to get started" : "Enter the OTP sent to your email"}</div>
        ...
        {step === "init" ? (
          <>
            ...
            <button style={{ ...s.btn, opacity: loading || !name || !email || !password ? 0.6 : 1 }} onClick={initSignUp} disabled={loading || !name || !email || !password}>
          ...
          <>
            ...
            <button style={{ ...s.btn, opacity: loading || !otp ? 0.6 : 1 }} onClick={completeSignUp} disabled={loading || !otp}>
        ...
        <div style={{ textAlign: "center", marginTop: 16, fontSize: 12, color: c.hint }}>
          Already have an account? <button disabled={loading} style={s.link} onClick={() => navigate("/login")}>Sign In</button>
        </div>
      </div>
    </div>
  );
